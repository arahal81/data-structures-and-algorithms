...
const count = (target, input) => {
  ...
  return input.reduce((acc, item) =>{
    return (acc + item.reduce((acc, item) => {return (item === target ? acc + 1 : acc);}, 0));}, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, item) =>{
    return (acc + item.reduce((acc, item) => {return (acc+item);}, 0));}, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray = input.map(item => item.filter(internalItem => (typeof(internalItem) === 'number') && internalItem%5===0));
  return newArray.map(item => item.map(internalItem => Math.pow(2, internalItem)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filterArray= data.filter(item => item.gender === 'male' || item.gender === 'female')
  return(filterArray.map(item => item.name).join(' and '));
...
let findShortest = (data) => {
  // Solution code here...
  let Shortest= data.reduce((acc, item)=> {
    if(acc.height < item.height)
    {acc=item;}
    return acc;}).name;
  return Shortest;
};
