...
// Solution code here...
  const newArr = arr.reduce((acc, item) => acc.length < item.length ? acc = item : acc, '');
  return arr.indexOf(newArr);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(item => item.split('').filter(item => /[0-9]/.test(item)).join(''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((item, i) => i % 2 !== 0).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.reduce((acc, item) => /:\)/.test(item) ? acc : acc = false, true);
};
